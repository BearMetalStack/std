...
A path is fine for `url`, it's just resolved against `http://localhost/` so relative links have
...
per-process and therefore exactly the wrong thing to route on so the router ignores it during a
render.
...
`typeof document !== "undefined"` no longer answers "am I in a browser" since a server render has a
`document`, which is the whole point of the Slag package. Use `isBrowser()` from `@bearmetal/app`:

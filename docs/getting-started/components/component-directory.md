...
The directory is found at either `components/` or `src/components/`, relative to where you start your server. The default bundle is served from `/@bearmetal/components/index` and its script tag is injected into every HTML response, so you never need to reference it yourself.
...
Every bundle is built in a single pass with code splitting on, so anything a subset shares with the default bundle — including the `@bearmetal/app` runtime and its signals — is hoisted into a shared chunk that each bundle imports. Adding a subset does not duplicate the components or the runtime it has in common with the others.

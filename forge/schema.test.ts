...
function ok<T>(schema: Schema<T>, value: unknown): T {
  ...
  if (!result.success) throw new Error(`Expected success, got: ${result.issues.map(i => i.message).join("; ")}`);
  return result.data;
}

function fail<T>(schema: Schema<T>, value: unknown): string[] {
  const result = schema.safeParse(value);
  if (result.success) throw new Error(`Expected failure, got success with: ${JSON.stringify(result.data)}`);

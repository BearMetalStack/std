...
import { createSignal, signalGetFn, type SignalNode, signalSetFn } from "./signal.ts";
...
export namespace Signal {
	...
	export namespace subtle {
		...
		export function introspectSources(sink: AnySink): AnySignal[] {
			...
			return (sink[NODE] as any).producerNode?.map((n: any) => n.wrapper) ?? [];
		}
		...
		export function introspectSinks(signal: AnySignal): AnySink[] {
			...
			return (signal[NODE] as any).liveConsumerNode?.map((n: any) => n.wrapper) ?? [];
		}
		...
		export function hasSinks(signal: AnySignal): boolean {
			...
			const liveConsumerNode = (signal[NODE] as any).liveConsumerNode;
		...
		export function hasSources(signal: AnySink): boolean {
			...
			const producerNode = (signal[NODE] as any).producerNode;
		...
		export class Watcher {
			...
			watch(...signals: AnySignal[]): void {
				...
				for (const signal of signals) {
					producerAccessed(signal[NODE] as any);
				}

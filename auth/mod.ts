import { Html, type Infer, Module, type Schema, type Service } from "@bearmetal/router";
...
export function authModule<T extends Schema<unknown>>(_userSchema?: T): Module<
	AuthState<Infer<T>>
> {
	...
	module
		...
		.route("/__auth/login")
		.get(async () => Html((await SignInPage()).toString()));

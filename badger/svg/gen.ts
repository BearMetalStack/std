...
function slashSeparator(
  ...
  const strip = `${x},0 ${x + shadow},0 ${x - skew + shadow},${h} ${
    x - skew
  },${h}`;
  return [
...
export function renderBadge(opts: BadgeOptions): string {
  ...
  if (isOutlined) {
    ...
    out.push(
      `  <rect x="0.5" y="0.5" width="${totalWidth - 1}" height="${
        height - 1
      }" rx="${
        radius - 0.5
    ...
    out.push(
      `  <rect x="0.5" y="0.5" width="${sectionWidths[0] - 0.5}" height="${
        height - 1
      }" rx="${radius - 0.5}" fill="${sections[0].bg}"/>`,
    );
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    out.push(
      `    <text x="${
        Math.round(cx)
      }" y="${textY}" text-anchor="middle" fill="${
